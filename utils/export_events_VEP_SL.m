function [events] = export_events_VEP_SL(Event_stream,times)
...
required_fields = {'type', 'latency', 'block', 'trial','trialtype'};%, 'answer', 'delay'};
required_types = {'str', 'num', 'str', 'num','str'};%, 'str', 'num'};
...
events = struct('type',[],'latency',[],'block',[],'trial',[],'trialtype',[]);
    ...
    if ~ismember({'event:Flip'},current_event)
        ...
        for m = 1:length(current_event)
            ...
            switch event_marker{1}
                ...
                case 'type'
                    events(t).trialtype = event_marker{2};
%                 otherwise
