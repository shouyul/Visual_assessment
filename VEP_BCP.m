...
% set subject info
subs = {'P1009','004-4012-s1','004-4012-s2','BCP003_6Hz','BCP003_12Hz',...
    'BCP004_6Hz','BCP004_12Hz','BCP005_6Hz','BCP005_12Hz'};
goggle_cond = {'GogglesON/12Hz','GogglesOFF/6Hz','GogglesON/12Hz',...
    'GogglesOFF/6Hz','GogglesON/12Hz','GogglesOFF/6Hz','GogglesON/12Hz',...
    'GogglesOFF/6Hz','GogglesON/12Hz'};
...
subIDs = {'BCP001','BCP002','BCP002','BCP003','BCP003',...
    'BCP004','BCP004','BCP005','BCP005'};
freq_cond = {'6Hz','6Hz','12Hz','6Hz','12Hz','6Hz','12Hz','6Hz','12Hz'};
% freq_cond = {'Low','Low','High','Low','High','Low','High','Low','High'};
...
for sub = 1:length(subs)
    ...
    Allsub_results(cnt).Subjid = subIDs{sub};
    Allsub_results(cnt).Task = 'VEP';
    Allsub_results(cnt).ElecGroup = 'All_electrodes';
    % Allsub_results(cnt).Condition = goggle_cond{sub};
    Allsub_results(cnt).Condition = freq_cond{sub};
...
filename = fullfile(result_fold,'Allsub_results_BCP.xlsx');
writetable(struct2table(Allsub_results),filename,"WriteMode","overwritesheet")
...
for sub = 1:length(subs)
    ...
    power_time(:,:,sub) = mean(results.power_time);
    dB_time(:,:,sub) = mean(results.dB_time);

end

power_time_avg = mean(power_time,3);
dB_time_avg = mean(dB_time,3);
...
times = 0:1/srate:10-1/srate;
contourf(times,results.freq,dB_time_avg,40,'linecolor','none');
clour_lim = 0.5;
