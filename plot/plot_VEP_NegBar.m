...
for el = 1:length(elecgroups)
    ...
    filename = sprintf('VEP_decoding_NegBar_%s.png',elecgroups{el});
...
filepath = '/Users/shouyuling/Desktop/Postdoc/Projects/Gensight/Data/VEPTask/Figures/SL/';
filename = sprintf('VEP_decoding_NegBar.png');
print(gcf,[filepath,filename],'-dpng', '-r300', '-painters');
...
order = categorical({'Control','-45Bar'});
...
for s = 1:length(subjids)
    for el = 1:length(elecgroups)
        for sg = 1:length(seg_con)
            ...
            filename = sprintf('VEP_conf_NegBar_%s_%s_%s',subjids{s},elecgroups{el},seg_con{sg});
