...
load('AllSub_class_error_binned.mat');
...
for el = 1:length(elecgroups)
    ...
    l = plot([0.5,2.5],[1/4,1/4]*100,'--','LineWidth',2,'Color',cus_cmap{4});
    ...
    filename = sprintf('VEP_decoding_binned_%s.png',elecgroups{el});
...
for s = 1:length(subjids)
    ...
    l = plot([0.5,5.5],[1/4,1/4]*100,'--','LineWidth',2,'Color',cus_cmap{4});
    ylabel('Classification accuracy (%)');
    ylim([20,45]);
    title(subjlabels{s});
    legend(segcon,'Location','bestoutside');
...
filepath = '/Users/shouyuling/Desktop/Postdoc/Projects/Gensight/Data/VEPTask/Figures/SL/';
filename = sprintf('VEP_decoding_binned.png');
print(gcf,[filepath,filename],'-dpng', '-r300', '-painters');
...
order = categorical({'Control','Disc','Bars','Rings'});
...
for s = 1:length(subjids)
    for el = 1:length(elecgroups)
        for sg = 1:length(seg_con)
            ...
            filename = sprintf('VEP_conf_binned_%s_%s_%s',subjids{s},elecgroups{el},seg_con{sg});
